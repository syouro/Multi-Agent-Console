...
Pantheon MCP server を user スコープで登録し、全 Claude Code セッションで利用できるようにします：
...
MCP server は最終的に以下へ転送します：

- `POST /api/pantheon/handoff`

HTTP MCP に対応したクライアントでは、以下を直接使うこともできます：
...
Codex CLI では stdio で同じ MCP server を登録できます。
